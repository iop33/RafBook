...
public class ChordState {
    ...
    private Map<Integer, DistributedFile> valueMap;
    private List<Integer> filesIAdded;
    // Friends
    private List<Integer> friendlyNodes = new CopyOnWriteArrayList<>();
    // Failed nodes
    ...
    public void addFriend(int friend) {
        friendlyNodes.add(friend);
    }
    
    public boolean isFriend(int serventPort) {
        return friendlyNodes.contains(serventPort);
    }
    ...
    public Map<Integer, DistributedFile> getValueMap() {
        return valueMap;
    }
    
    public void setValueMap(Map<Integer, DistributedFile> valueMap) {
    ...
    public void addFileToFilesIAdded(int chordId) {
        filesIAdded.add(chordId);
    }
    
    public List<Integer> getFilesIAdded() {
        return filesIAdded;
    }
    
    public void removeFilesFromFilesIAdded(int key) {
        filesIAdded.remove(key);
    }
    ...
    private void updateSuccessorTable() {
        ...
        for (int i = 1; i < chordLevel; i++, currentIncrement *= 2) {
            ...
            int currentValue =
                    (AppConfig.myServentInfo.getChordId() + currentIncrement) % CHORD_SIZE;
            ...
            while (true) {
                if (currentValue > currentId) {
                    ...
                    ServentInfo nextNode =
                            allNodeInfo.get((currentNodeIndex + 1) % allNodeInfo.size());
        ...
        if (PRINT_DEBUG) {
            AppConfig.timestampedStandardPrint(
                    "New chord state: " + AppConfig.chordState + " successorTable: "
                            + Arrays.toString(successorTable) + "\nMy values:\n" + valueMap);
        }
    }
     ...
     */
    public void putValue(int key, DistributedFile value) {
        if (isKeyMine(key)) {
            valueMap.put(key, value);
            if (PRINT_DEBUG) {
                AppConfig.timestampedStandardPrint(
                        "[File added] New chord state: " + AppConfig.chordState
                                + " successorTable: " + Arrays.toString(successorTable)
                                + "\nMy values:\n" + valueMap);
            }
        } else {
            ServentInfo nextNode = getNextNodeForKey(key);
            AddFileMessage pm = new AddFileMessage(AppConfig.myServentInfo.getListenerPort(),
                    nextNode.getListenerPort(), key, value);
     ...
     * </ul>
     */
    public DistributedFile getValue(int key) {
        if (isKeyMine(key)) {
            if (valueMap.containsKey(key)) {
                DistributedFile file = valueMap.get(key);
                if (!file.isPublic() && isFriend(file.getOwnerPort())) {
                    return new DistributedFile("NOT_FRIEND", false, -1, -1);
                }
        ...
        AskGetMessage agm = new AskGetMessage(AppConfig.myServentInfo.getListenerPort(),
                nextNode.getListenerPort(), String.valueOf(key), friendlyNodes);
        MessageUtil.sendMessage(agm);
        
        return new DistributedFile("WAIT", true, -1, -1);
    }
    
    public DistributedFile getFileValue(int key) {
        if (isKeyMine(key)) {
            return valueMap.get(key);
        } else {
            ServentInfo nextNode = getNextNodeForKey(key);
    ...
    public void removeFileWithPath(String filePath) {
        int keyToRemove = chordHash(filePath);
        if (isKeyMine(keyToRemove)) {
            // FileRemovedMessage fileRemovedMessage = new FileRemovedMessage(AppConfig.myServentInfo.getListenerPort(), valueMap.get(keyToRemove).getOwnerPort(), keyToRemove);
            // MessageUtil.sendMessage(fileRemovedMessage);
            DistributedFile removedFile = valueMap.remove(keyToRemove);
            if (PRINT_DEBUG) {
                AppConfig.timestampedStandardPrint(
                        "Removed file: " + filePath + ", New chord state: " + AppConfig.chordState
                                + " successorTable: " + Arrays.toString(successorTable)
                                + "\nMy values:\n" + valueMap);
            }
        } else {
            ...
            if (PRINT_DEBUG) {
                AppConfig.timestampedStandardPrint("Propagating remove file action to " + nextNode);
            }
            RemoveFileMessage removeFileMessage =
                    new RemoveFileMessage(AppConfig.myServentInfo.getListenerPort(),
                            nextNode.getListenerPort(), filePath);
    ...
    public void handleNodeFailure(int failedNodePort) {
        try {
            ...
            allNodeInfo.addAll(newList);
            allNodeInfo.addAll(newList2);
            AppConfig.timestampedStandardPrint("After remove nodes: " + allNodeInfo);
            if (newList2.size() > 0) {
                predecessorInfo = newList2.get(newList2.size() - 1);
            } else {
                predecessorInfo = newList.get(newList.size() - 1);
    ...
    public void addBackup(Map<Integer, DistributedFile> backup) {
