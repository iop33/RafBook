...
public class NewNodeHandler implements MessageHandler {
		...
		if (clientMessage.getMessageType() == MessageType.NEW_NODE) {
			...
			if (isMyPred) { //if yes, prepare and send welcome message
				...
				synchronized (MutexState.lock) {
					...
					Map<Integer, DistributedFile> myValues = AppConfig.chordState.getValueMap();
					Map<Integer, DistributedFile> hisValues = new HashMap<>();

					int myId = AppConfig.myServentInfo.getChordId();
					int hisPredId = hisPred.getChordId();
					int newNodeId = newNodeInfo.getChordId();

					for (Entry<Integer, DistributedFile> valueEntry : myValues.entrySet()) {
	...
	private void waitForReorganizedMessage(){
		while(!MutexState.isReorganizationDone.get()){
			try {
				Thread.sleep(200);
		...
		MutexState.isReorganizationDone.set(false);
	}
