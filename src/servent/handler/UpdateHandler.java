...
import servent.message.organization.DoneReorganizingMessage;
	...
	public void run() {
		if (clientMessage.getMessageType() == MessageType.UPDATE) {
			if (clientMessage.getSenderPort() != AppConfig.myServentInfo.getListenerPort()) {
				...
				// Lock so it doesn't clash with add/remove node
				...
				// Send done reorganizing message to successor that added me to system
				DoneReorganizingMessage message = new DoneReorganizingMessage(AppConfig.myServentInfo.getListenerPort(), AppConfig.chordState.getNextNodePort());
