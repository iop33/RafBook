...
import cli.command.files.GetFilesFromPortCommand;
import cli.command.files.RemoveFileCommand;
import cli.command.files.ViewFileCommand;
...
public class CLIParser implements Runnable, Cancellable {
	...
	public CLIParser(SimpleServentListener listener, DistributedMutex mutex, Backuper backuper) {
		...
		commandList.add(new GetFilesFromPortCommand());
		commandList.add(new AddFileCommand());
		commandList.add(new StopCommand(this, listener, backuper));
		commandList.add(new AddFriendCommand());
		commandList.add(new EnterSystemCommand());
